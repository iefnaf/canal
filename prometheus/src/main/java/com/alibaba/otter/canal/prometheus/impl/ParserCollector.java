...
public class ParserCollector extends Collector implements InstanceRegistry {
  ...
  private void singleCollect(CounterMetricFamily bytesCounter, CounterMetricFamily blockingCounter,
      GaugeMetricFamily modeGauge, ParserMetricsHolder holder) {
    if (holder.isParallel) {
      blockingCounter.addMetric(holder.parserLabelValues,
          (holder.eventsPublishBlockingTime.doubleValue() / NANO_PER_MILLI));
    }
  ...
  public void register(CanalInstance instance) {
    ...
    if (parser instanceof AbstractMysqlEventParser) {
      ...
      throw new IllegalArgumentException(
          "CanalEventParser must be either AbstractMysqlEventParser or GroupEventParser.");
    }
  ...
  private ParserMetricsHolder singleHolder(String destination, AbstractMysqlEventParser parser,
      String id) {
  ...
  private GroupParserMetricsHolder groupHolder(String destination, GroupEventParser group) {
    ...
    for (int i = 0; i < num; i++) {
      ...
      if (parser instanceof AbstractMysqlEventParser) {
        ParserMetricsHolder single = singleHolder(destination, (AbstractMysqlEventParser) parser,
