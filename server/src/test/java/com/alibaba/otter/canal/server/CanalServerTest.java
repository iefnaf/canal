  ...
  public void testAuth() {
    ...
    try {
      ...
      if (ack.getErrorCode() > 0) {
        throw new Exception(
            "something goes wrong when doing authentication: " + ack.getErrorMessage());
      }
      ...
      writeWithHeader(channel, Packet.newBuilder()
          ...
          .setVersion(NettyUtils.VERSION)
          .setBody(
              Sub.newBuilder().setDestination(DESTINATION).setClientId("1").build().toByteString())
          .build()
          .toByteArray());
      //
      p = Packet.parseFrom(readNextPacket(channel));
      ...
      writeWithHeader(channel,
          Packet.newBuilder()
              ...
              .setVersion(NettyUtils.VERSION)
              .setBody(Unsub.newBuilder().setDestination(DESTINATION).setClientId("1").build()
                  .toByteString())
              .build()
              .toByteArray());
