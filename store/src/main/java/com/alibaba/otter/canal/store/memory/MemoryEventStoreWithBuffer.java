...
public class MemoryEventStoreWithBuffer extends AbstractCanalStoreScavenge implements
  ...
  private AtomicLong putSequence = new AtomicLong(
      INIT_SEQUENCE);             // 代表当前put操作最后一次写操作发生的位置
  ...
  public boolean put(Event data, long timeout, TimeUnit unit)
      throws InterruptedException, CanalStoreException {
  ...
  private Events<Event> doGet(Position start, int batchSize) throws CanalStoreException {
    ...
    if (startPosition == null || !startPosition.getPostion()
        .isIncluded()) { // 第一次订阅之后，需要包含一下start位置，防止丢失第一条记录
      next = next + 1;
    }
    ...
    for (int i = entrys.size() - 1; i >= 0; i--) {
      ...
      if ((CanalEntry.EntryType.TRANSACTIONBEGIN == event.getEntryType() && StringUtils.isEmpty(
          event.getGtid()))
          || CanalEntry.EntryType.TRANSACTIONEND == event.getEntryType() || isDdl(
          event.getEventType())) {
        // 将事务头/尾设置可被为ack的点
  ...
  public void cleanUntil(Position position, Long seqId) throws CanalStoreException {
    ...
    try {
      ...
      for (long next = sequence + 1; next <= maxSequence; next++) {
        ...
        memsize += calculateSize(event);
        if ((seqId < 0 || next == seqId) && CanalEventUtils.checkPosition(event,
