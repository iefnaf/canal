...
public class HbaseConfigMonitor {
  ...
  private class FileListener extends FileAlterationListenerAdaptor {
    ...
    public void onFileCreate(File file) {
      ...
      try {
        // 加载新增的配置文件
        String configContent = MappingConfigsLoader.loadConfig(
            adapterName + File.separator + file.getName());
        MappingConfig config = YamlUtils.ymlToObj(null, configContent, MappingConfig.class, null,
            envProperties);
        if (config == null) {
          return;
        }
