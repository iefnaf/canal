...
public class PhTypeUtil {
  ...
  public static Object toObject(byte[] b, PhType phType) {
    ...
    if (phType == PhType.INTEGER) {
      ...
      ts.setNanos(
          nanosDeserialized < 1000000 ? ts.getNanos() + nanosDeserialized : nanosDeserialized);
      v = ts;
    } else if (phType == PhType.UNSIGNED_TIME || phType == PhType.UNSIGNED_DATE) {
  ...
  private static final MathContext DEFAULT_MATH_CONTEXT = new MathContext(MAX_PRECISION,
