...
public class HbaseSyncService {
   ...
   * @param data         Map数据
   */
  private static void convertData2Row(MappingConfig.HbaseMapping hbaseMapping, HRow hRow,
      Map<String, Object> data) {
    Map<String, MappingConfig.ColumnItem> columnItems = hbaseMapping.getColumnItems();
    int i = 0;
    for (Map.Entry<String, Object> entry : data.entrySet()) {
      if (hbaseMapping.getExcludeColumns() != null && hbaseMapping.getExcludeColumns()
  ...
  private static byte[] getRowKeyBytes(MappingConfig.HbaseMapping hbaseMapping,
      MappingConfig.ColumnItem rowKeyColumn,
      Map<String, Object> rowData) {
    Object val = rowData.get(rowKeyColumn.getColumn());
    String v = null;
    if (rowKeyColumn.getRowKeyLen() != null) {
      if (val instanceof Number) {
        v = String.format("%0" + rowKeyColumn.getRowKeyLen() + "d",
            (Number) ((Number) val).longValue());
