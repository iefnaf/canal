...
public class SqlParser {
  ...
  public static SchemaItem parse(String sql) {
    try {
      ...
      tableItems.forEach(
          tableItem -> schemaItem.getAliasTableItems().put(tableItem.getAlias(), tableItem));

      List<FieldItem> fieldItems = collectSelectQueryFields(sqlSelectQueryBlock);
      fieldItems.forEach(
          fieldItem -> schemaItem.getSelectFields().put(fieldItem.getFieldName(), fieldItem));
   ...
   * @return 字段属性列表
   */
  private static List<FieldItem> collectSelectQueryFields(
  ...
  private static void visitSelectTable(SchemaItem schemaItem, SQLTableSource sqlTableSource,
    ...
    if (sqlTableSource instanceof SQLExprTableSource) {
      ...
      MySqlSelectQueryBlock sqlSelectQuery = (MySqlSelectQueryBlock) subQueryTableSource.getSelect()
          .getQuery();
      TableItem tableItem;
      if (tableItemTmp != null) {
        tableItem = tableItemTmp;
  ...
  private static void visitOnCondition(SQLExpr expr, TableItem tableItem) {
    ...
    if (sqlBinaryOpExpr.getOperator() == BooleanAnd) {
      ...
      if (leftFieldItem.getColumnItems().size() != 1 || leftFieldItem.isMethod()
          || leftFieldItem.isBinaryOp()) {
