...
public class TypeUtil {
  ...
  public static String getPhoenixType(SQLColumnDefinition definition, boolean limit) {
    ...
    SQLDataTypeImpl sqlDataType1 =
        sqlDataType instanceof SQLDataTypeImpl ? (SQLDataTypeImpl) sqlDataType : null;
    boolean isUnsigned = sqlDataType1 != null && sqlDataType1.isUnsigned();

    return getPhoenixType(sqlDataType.getName().toUpperCase(), sqlDataType.getArguments().toArray(),
        isUnsigned, limit);
  }

  //MySQL类型和Phoenix类型一一对应
  public static String getPhoenixType(String name, Object[] args, boolean isUnsigned,
      boolean limit) {
    switch (name) {
      case "BIT":
        if (limit) {
