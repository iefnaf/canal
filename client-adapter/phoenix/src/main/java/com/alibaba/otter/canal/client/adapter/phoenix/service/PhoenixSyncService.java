...
public class PhoenixSyncService {
  ...
  public void sync(Map<String, Map<String, MappingConfig>> mappingConfig, List<Dml> dmls,
    ...
    sync(dmls, dml -> {
      ...
      if (envProperties != null && !"tcp".equalsIgnoreCase(
          envProperties.getProperty("canal.conf.mode"))) {
        //tcp 模式
      ...
      if (configMap == null) {
        if (logger.isTraceEnabled()) {
          logger.trace("no config map: destination={},groupId={}, database={}, table={}, keys={}",
              destination, groupId, database, table, mappingConfig.keySet());
        }
        return false;
      }
      if (configMap.values().isEmpty()) {
        logger.info("config map empty: destination={},groupId={}, database={}, table={}, keys={}",
            destination, groupId, database, table, mappingConfig.keySet());
        return false;
      }
  ...
  private void alter(BatchExecutor batchExecutor, MappingConfig config, Dml dml,
      List<SQLStatement> stmtList, String configFile) throws SQLException {
    if (config.isDebug()) {
      ...
      if (statement instanceof SQLAlterTableStatement) {
        ...
        for (SQLAlterTableItem item : alterTable.getItems()) {
          if (item instanceof SQLAlterTableDropColumnItem) {
            ...
            if (!dbMapping.isDrop()) {
              logger.info("drop table column disabled: {} {}", targetTable,
            ...
            for (SQLColumnDefinition definition : addColumn.getColumns()) {
              ...
              String sql = "ALTER TABLE " + targetTable +
                  " ADD IF NOT EXISTS " +
                  dbMapping.escape(name) + " " + TypeUtil.getPhoenixType(definition,
                  dbMapping.isLimit());
              try {
                ...
                if (definition.getDefaultExpr() != null) {
                  ...
                  if (!defVal.equalsIgnoreCase("NULL") && !defVal.equalsIgnoreCase("NOT NULL")
                      && name.length() > 0) {
    ...
    if (!defValues.isEmpty()) {
      ...
      try {
        logger.info("set column default value: {} {}", defSql,
            batchExecutor.executeUpdate(defSql.toString()));
  ...
  private void insert(BatchExecutor batchExecutor, MappingConfig config, SingleDml dml)
    ...
    for (Map.Entry<String, String> entry : columnsMap.entrySet()) {
      ...
      if (type == null) {
        if (dbMapping.isSkipMissing()) {
          ...
          mapLen -= 1;
          continue;
        } else if (dbMapping.getMapAll() && dbMapping.isAlter() && PhoenixEtlService.syncSchema(
            batchExecutor.getConn(), config)) {
          columnsTypeCache.remove(
              config.getDestination() + "." + dbMapping.getDatabase() + "." + dbMapping.getTable());
    ...
    try {
      if (old != null && !old.isEmpty()) {
        ...
        deleteSql.append("DELETE FROM ").append(SyncUtil.getDbTableName(dbMapping))
            .append(" WHERE ");
