...
public class PhoenixEtlService {
  ...
  private static boolean syncSchema(DataSource srcDS, Connection targetDSConnection,
  ...
  private static boolean syncSchema(Connection srcDS, Connection targetDS, MappingConfig config) {
    ...
    if (dbMapping.getMapAll() && dbMapping.isAlter()) { // 检查字段是否缺失
      ...
      Util.sqlRS(srcDS, "SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '"
          + dbMapping.getDatabase() + "'  AND TABLE_NAME = '" + dbMapping.getTable() + "'", rs -> {
        try {
          ...
          while (rs.next()) {
            ...
            if (targetColumnType.get(lower) == null && !excludeColumns.contains(lower)) {
              ...
              String[] args = splitNotEmpty(
                  colType.replaceAll("^\\w+(?:\\(([^)]*)\\))?[\\s\\S]*$", "$1"));
              ...
              if (isPri) {
                if (args.length > 0 && dbMapping.isLimit() || rs.getString("IS_NULLABLE")
                    .equals("NO")) {
          ...
          logger.error(dbMapping.getDatabase() + "." + dbMapping.getTable() + " schema failed! ==>"
              + e.getMessage(), e);
          throw new RuntimeException(e);
        }
      });
   ...
   * 导入数据
   */
  public static EtlResult importData(DataSource srcDS, Connection targetDSConnection,
      MappingConfig config,
      List<String> params) {
    ...
    try {
      ...
      if (cnt >= 10000) {
        ...
        for (int i = 0; i < threadCount; i++) {
          ...
          executor
              .execute(() -> executeSqlImport(srcDS, targetDSConnection, sqlFinal, dbMapping,
                  successCount, errMsg, debug));
        }
        ...
        executeSqlImport(srcDS, targetDSConnection, sql.toString(), dbMapping, successCount, errMsg,
            debug);
      }

      logger.info(
          dbMapping.getTable() + " etl completed in: " + (System.currentTimeMillis() - start) / 1000
              + "s!");

      etlResult
          .setResultMessage(
              "导入目标表 " + SyncUtil.getDbTableName(dbMapping) + " 数据：" + successCount.get()
                  + " 条");
    } catch (Exception e) {
      logger.error(e.getMessage(), e);
   ...
   * 执行导入
   */
  private static boolean executeSqlImport(DataSource srcDS, Connection targetDSConnection,
      String sql, DbMapping dbMapping,
      AtomicLong successCount, List<String> errMsg, boolean debug) {
    try {
      Map<String, String> columnsMap = new LinkedHashMap<>();
      Map<String, Integer> columnType = new LinkedHashMap<>();

      PhoenixSupportUtil.sqlRS(targetDSConnection,
          "SELECT * FROM " + SyncUtil.getDbTableName(dbMapping) + " LIMIT 1 ", rs -> {
            try {
      ...
      Util.sqlRS(srcDS, sql, rs -> {
        ...
        try {
          ...
          columnsMap
              .forEach((targetColumnName, srcColumnName) -> insertSql.append(
                  dbMapping.escape(targetColumnName)).append(","));
   ...
   * 拼接目标表主键where条件
   */
  private static void appendCondition(DbMapping dbMapping, StringBuilder sql,
