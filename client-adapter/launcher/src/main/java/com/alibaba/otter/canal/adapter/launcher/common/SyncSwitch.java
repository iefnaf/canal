...
public class SyncSwitch {
  ...
  private synchronized void initMutex(CuratorFramework curator, String destination,
      BooleanMutex mutex) {
    try {
      String path = SYN_SWITCH_ZK_NODE + destination;
    ...
    if (mode == Mode.LOCAL) {
      ...
      try {
        ...
        try {
          ...
          curatorClient.getCurator().setData()
              .forPath(path, "off".getBytes(StandardCharsets.UTF_8));
        }
      } catch (Exception e) {
  ...
  public void get(String destination, long timeout, TimeUnit unit)
