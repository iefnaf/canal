...
public class TablestoreEtlService extends AbstractEtlService {
  ...
  public EtlResult importData(List<String> params) {
    ...
    String sql =
        "SELECT * FROM " + SyncUtil.getDbTableName(dbMapping.getDatabase(), dbMapping.getTable());
        ...
        for (Future<WriterResult> future : futureList) {
          try {
            ...
            if (result != null && result.isAllSucceed()) {
              ...
              List<String> msgs = totalFailedRows.stream()
                  .map(e -> TablestoreAdapter.buildErrorMsgForFailedRowChange(e))
                  .collect(Collectors.toList());
              logger.error("Failed rows when ETL:"
                  + org.springframework.util.StringUtils.collectionToDelimitedString(msgs, ",", "[",
                  "]"));
            }
