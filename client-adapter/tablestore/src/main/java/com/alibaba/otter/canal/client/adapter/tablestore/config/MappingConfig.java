    ...
    public void init(MappingConfig config) {
      ...
      if (targetColumns != null) {
        ...
        if (needTypeInference) {
          ...
          Util.sqlRS(sourceDS,
              "SELECT * FROM " + SyncUtil.getDbTableName(database, table) + " LIMIT 1 ", rs -> {
                try {
                  ...
                  for (int i = 1; i <= columnCount; i++) {
                    ...
                    if (columnItems.containsKey(columnName)
                        && columnItems.get(columnName).getType() == null) {
                      int columnType = rsd.getColumnType(i);
                      columnItems.get(columnName)
                          .setType(SyncUtil.getDefaultTablestoreType(columnType));
                    }
                  }
                  return true;
                } catch (Exception e) {
