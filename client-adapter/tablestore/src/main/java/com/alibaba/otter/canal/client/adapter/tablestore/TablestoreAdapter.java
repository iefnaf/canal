...
public class TablestoreAdapter implements OuterAdapter {
  ...
  public void init(OuterAdapterConfig configuration, Properties envProperties) {
    ...
    if (tablestoreMapping.isEmpty()) {
      throw new RuntimeException(
          "No tablestore adapter found for config key: " + configuration.getKey());
    }
  ...
  public void sync(List<Dml> dmls) {
    ...
    try {
      ...
      if (!CollectionUtils.isEmpty(totalFailedRows)) {
        // 认为有失败的请求
        List<String> msgs = totalFailedRows.stream().map(e -> buildErrorMsgForFailedRowChange(e))
            .collect(Collectors.toList());
        throw new RuntimeException(
            "Failed rows:" + org.springframework.util.StringUtils.collectionToDelimitedString(msgs,
                ",", "[", "]"));
      }
   ...
   * @return
   */
  public static String buildErrorMsgForFailedRowChange(
      WriterResult.RowChangeStatus rowChangeStatus) {
   ...
   * @return
   */
  private TableStoreWriter buildEtlWriter(OuterAdapterConfig configuration,
  ...
  private void addSyncConfigToCache(String configName, MappingConfig mappingConfig) {
    ...
    if (envProperties != null && !"tcp".equalsIgnoreCase(
      ...
      key = StringUtils.trimToEmpty(mappingConfig.getDestination()) + "-"
          ...
          + mappingConfig.getDbMapping().getDatabase() + "-" + mappingConfig.getDbMapping()
          .getTable();
    } else {
