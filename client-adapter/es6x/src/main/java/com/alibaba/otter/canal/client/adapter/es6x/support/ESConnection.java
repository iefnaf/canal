...
public class ESConnection {
  ...
  public ESConnection(String[] hosts, Map<String, String> properties, ESClientMode mode)
    ...
    if (mode == ESClientMode.TRANSPORT) {
      ...
      for (String host : hosts) {
        ...
        transportClient.addTransportAddress(
            new TransportAddress(InetAddress.getByName(host.substring(0, i)),
                Integer.parseInt(host.substring(i + 1))));
      }
    } else {
      HttpHost[] httpHosts = Arrays.stream(hosts).map(this::createHttpHost)
      ...
      if (StringUtils.isNotEmpty(nameAndPwd) && nameAndPwd.contains(":")) {
        ...
        credentialsProvider.setCredentials(AuthScope.ANY,
            new UsernamePasswordCredentials(nameAndPwdArr[0],
                nameAndPwdArr[1]));
  ...
  public MappingMetaData getMapping(String index, String type) {
    ...
    if (mode == ESClientMode.TRANSPORT) {
      ...
      try {
        ...
        response = RestHighLevelClientExt.getMapping(restHighLevelClient, request,
  ...
  private HttpHost createHttpHost(String uriStr) {
    ...
    try {
      return HttpHost.create(
          new URI(uri.getScheme(), null, uri.getHost(), uri.getPort(), uri.getPath(),
