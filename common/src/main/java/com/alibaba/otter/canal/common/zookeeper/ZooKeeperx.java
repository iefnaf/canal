...
public class ZooKeeperx extends ZkConnection {
  ...
  private static final Field hostProviderField = ReflectionUtils.findField(ClientCnxn.class,
      "hostProvider");
  private static final Field serverAddressesField = ReflectionUtils.findField(
    ...
    try {
      if (_serversList.size() > 1) {
        ...
        for (int i = 1; i < _serversList.size(); i++) {
          ...
          HostProvider hostProvider = (HostProvider) ReflectionUtils.getField(hostProviderField,
              cnxn);
