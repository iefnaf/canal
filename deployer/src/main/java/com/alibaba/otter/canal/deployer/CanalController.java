...
public class CanalController {
  ...
  public CanalController(final Properties properties) {
    ...
    int metricsPort = Integer.valueOf(
        getProperty(properties, CanalConstants.CANAL_METRICS_PULL_PORT, "11112"));
            ...
            if (runningMonitor.isStart()) {
              ...
              if (!release) {
                ...
                if (instanceConfigMonitors.containsKey(InstanceConfig.InstanceMode.MANAGER)) {
                  ManagerInstanceConfigMonitor monitor = (ManagerInstanceConfigMonitor) instanceConfigMonitors.get(
                      InstanceConfig.InstanceMode.MANAGER);
  ...
  private InstanceConfig initGlobalConfig(Properties properties) {
    ...
    String modeStr = getProperty(properties,
        CanalConstants.getInstanceModeKey(CanalConstants.GLOBAL_NAME));
    ...
    String lazyStr = getProperty(properties,
        CanalConstants.getInstancLazyKey(CanalConstants.GLOBAL_NAME));
    ...
    String springXml = getProperty(properties,
        CanalConstants.getInstancSpringXmlKey(CanalConstants.GLOBAL_NAME));
  ...
  private void initInstanceConfig(Properties properties) {
    ...
    for (String destination : destinations) {
      ...
      if (oldConfig != null) {
        logger.warn("destination:{} old config:{} has replace by new config:{}", destination,
            oldConfig, config);
      }
    }
  }
  ...
  private InstanceConfig parseInstanceConfig(Properties properties, String destination) {
    ...
    if (config.getMode().isManager()) {
      String managerAddress = getProperty(properties,
          CanalConstants.getInstanceManagerAddressKey(destination));
      if (StringUtils.isNotEmpty(managerAddress)) {
        if (StringUtils.equals(managerAddress, "${canal.admin.manager}")) {
          managerAddress = adminManagerAddress;
        }
  ...
  public void stop() throws Throwable {
    ...
    for (ServerRunningMonitor runningMonitor : ServerRunningMonitors.getRunningMonitors()
        .values()) {
      if (runningMonitor.isStart()) {
        runningMonitor.stop();
      }
