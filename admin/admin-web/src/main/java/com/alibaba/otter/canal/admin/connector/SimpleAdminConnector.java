...
public class SimpleAdminConnector implements AdminConnector {
  ...
  private int idleTimeout =
      60 * 60 * 1000;                                     // client和server之间的空闲链接超时的时间,默认为1小时
    ...
    try {
      ...
      if (passwd != null) {
        ...
        newPasswd = SecurityUtil.byte2HexStr(
            SecurityUtil.scramble411(passwd.getBytes(), seed.toByteArray()));
      }
      ...
      if (ackBody.getCode() > 0) {
        throw new ServiceException(
            "something goes wrong when doing authentication: " + ackBody.getMessage());
      }
  ...
  private String doLogAdmin(String type, String action, String destination, String file,
      int count) {
    try {
      writeWithHeader(Packet.newBuilder()
