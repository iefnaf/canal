...
public class NodeServerServiceImpl implements NodeServerService {
  ...
  public void delete(Long id) {
    ...
    if (nodeServer != null) {
      // 判断是否存在实例
      int cnt = CanalInstanceConfig.find.query().where().eq("serverId", nodeServer.getId())
          .findCount();
      if (cnt > 0) {
  ...
  public Pager<NodeServer> findList(NodeServer nodeServer, Pager<NodeServer> pager) {
    ...
    for (NodeServer ns : nodeServers) {
      futures.add(Threads.executorService.submit(() -> {
        boolean status = SimpleAdminConnectors.execute(ns.getIp(), ns.getAdminPort(),
            AdminConnector::check);
  ...
  public boolean remoteOperation(Long id, String option) {
    ...
    if ("start".equals(option)) {
      result = SimpleAdminConnectors.execute(nodeServer.getIp(), nodeServer.getAdminPort(),
