...
public class CanalKafkaClientFlatMessageExample {
  ...
  protected final static Logger logger = LoggerFactory.getLogger(
      CanalKafkaClientFlatMessageExample.class);
  ...
  private Thread.UncaughtExceptionHandler handler = (t, e) -> logger.error(
      "parse events has an error", e);

  public CanalKafkaClientFlatMessageExample(String zkServers, String servers, String topic,
      Integer partition,
      String groupId) {
  ...
  public static void main(String[] args) {
    try {
      ...
      logger.info("## start the kafka consumer: {}-{}", AbstractKafkaTest.topic,
          AbstractKafkaTest.groupId);
  ...
  private void process() {
    ...
    while (running) {
      try {
        ...
        while (running) {
          try {
            List<FlatMessage> messages = connector.getFlatList(100L,
                TimeUnit.MILLISECONDS); // 获取message
            if (messages == null) {
              continue;
            }
