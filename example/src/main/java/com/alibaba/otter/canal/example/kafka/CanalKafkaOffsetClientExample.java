...
public class CanalKafkaOffsetClientExample {
  ...
  protected final static Logger logger = LoggerFactory.getLogger(
      CanalKafkaOffsetClientExample.class);
  ...
  public CanalKafkaOffsetClientExample(String servers, String topic, Integer partition,
      String groupId) {
      ...
      try {
        ...
        while (running) {
          try {
            ...
            List<KafkaMessage> messages = connector.getListWithoutAck(100L, TimeUnit.MILLISECONDS,
                offset); // 获取message
            if (messages == null) {
              continue;
            }
