...
public class DirectLogFetcherTest {
  ...
  private void sendBinlogDump(MysqlConnector connector, String binlogfilename, Long binlogPosition,
      int slaveId)
      throws IOException {
  ...
  private void loadBinlogChecksum(MysqlConnector connector) {
    ...
    if (columnValues != null && columnValues.size() >= 1 && columnValues.get(0).toUpperCase()
        .equals("CRC32")) {
  ...
  protected void parseRowsQueryEvent(RowsQueryLogEvent event) throws Exception {
    ...
    System.out.println(
        "sql : " + new String(event.getRowsQuery().getBytes("ISO-8859-1"), charset.name()));
  }
  ...
  public void parseTableMapEvent(TableMapLogEvent event) {
    try {
      ...
      String charsetTbName = new String(event.getTableName().getBytes("ISO-8859-1"),
          charset.name());
