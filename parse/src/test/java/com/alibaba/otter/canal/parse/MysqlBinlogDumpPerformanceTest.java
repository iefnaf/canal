...
public class MysqlBinlogDumpPerformanceTest {
  ...
  public static void main(String args[]) {
    ...
    controller.setMasterInfo(
        new AuthenticationInfo(new InetSocketAddress("100.81.154.142", 3306), "canal", "canal"));
    ...
    controller.setEventSink(new AbstractCanalEventSinkTest<List<CanalEntry.Entry>>() {
      ...
      public boolean sink(List<CanalEntry.Entry> entrys, InetSocketAddress remoteAddress,
          String destination)
        ...
        if (current - last.get() >= 100000) {
          ...
          System.out.println(
              " total : " + sum + " , cost : " + (end.get() - start.get()) + " , tps : " + tps);
          last.set(current);
          start.set(end.get());
        }
        return true;
  ...
  public static abstract class AbstractCanalEventSinkTest<T> extends
      AbstractCanalLifeCycle implements CanalEventSink<T> {
