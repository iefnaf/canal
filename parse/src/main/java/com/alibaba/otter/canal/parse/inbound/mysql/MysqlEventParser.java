 ...
 * @version 1.0.0
 */
public class MysqlEventParser extends AbstractMysqlEventParser implements CanalEventParser,
    CanalHASwitchable {
  ...
  protected EntryPosition findPositionWithMasterIdAndTimestamp(MysqlConnection connection,
      EntryPosition fixedPosition) {
    MysqlConnection mysqlConnection = (MysqlConnection) connection;
    if (tableMetaTSDB != null && (fixedPosition.getTimestamp() == null
        || fixedPosition.getTimestamp() <= 0)) {
      // 使用一个未来极大的时间，基于位点进行定位
      long startTimestamp =
          System.currentTimeMillis() + 102L * 365 * 24 * 3600 * 1000; // 当前时间的未来102年
  ...
  protected EntryPosition findStartPositionInternal(ErosaConnection connection) {
    ...
    if (logPosition == null) {// 找不到历史成功记录
      ...
      if (masterInfo != null && mysqlConnection.getConnector().getAddress()
      ...
      if (logPosition.getIdentity().getSourceAddress()
   ...
   * @return
   */
  protected EntryPosition fallbackFindByStartTimestamp(LogPosition logPosition,
  ...
  private Long findTransactionBeginPosition(ErosaConnection mysqlConnection,
      final EntryPosition entryPosition)
      throws IOException {
    // 针对开始的第一条为非Begin记录，需要从该binlog扫描
    final java.util.concurrent.atomic.AtomicLong preTransactionStartPosition = new java.util.concurrent.atomic.AtomicLong(
        0L);
    mysqlConnection.reconnect();
    mysqlConnection.seek(entryPosition.getJournalName(), 4L, entryPosition.getGtid(),
        new SinkFunction<LogEvent>() {
          ...
          public boolean sink(LogEvent event) {
            try {
              ...
              processSinkError(e, lastPosition, entryPosition.getJournalName(),
      ...
      try {
        ...
        if (entryPosition == null) {
          if (StringUtils.equalsIgnoreCase(minBinlogFileName, startSearchBinlogFile)) {
            ...
            // 继续往前找
            int binlogSeqNum = Integer.parseInt(
                startSearchBinlogFile.substring(startSearchBinlogFile.indexOf(".") + 1));
            if (binlogSeqNum <= 1) {
        ...
        logger.warn(String.format(
                "the binlogfile:%s doesn't exist, to continue to search the next binlogfile , caused by",
                startSearchBinlogFile),
            e);
        int binlogSeqNum = Integer.parseInt(
            startSearchBinlogFile.substring(startSearchBinlogFile.indexOf(".") + 1));
  ...
  private SlaveEntryPosition findSlavePosition(MysqlConnection mysqlConnection) {
    try {
      ...
      if ((!"0".equals(errno)) || (!"Yes".equalsIgnoreCase(slaveIORunning))
        ...
        logger.warn("Ignoring failed slave: " + mysqlConnection.getConnector().getAddress()
            + ", Last_Errno = "
  ...
  private EntryPosition findAsPerTimestampInSpecificLogFile(MysqlConnection mysqlConnection,
    ...
    try {
      mysqlConnection.reconnect();
      // 开始遍历文件
      mysqlConnection.seek(searchBinlogFile, 4L, endPosition.getGtid(),
          new SinkFunction<LogEvent>() {
            ...
            public boolean sink(LogEvent event) {
              ...
              try {
                CanalEntry.Entry entry = parseAndProfilingIfNecessary(event, true);
                if (justForPositionTimestamp && logPosition.getPostion() == null
                    && event.getWhen() > 0) {
                  // 初始位点
