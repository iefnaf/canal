 ...
 * @version 1.0.0
 */
public abstract class AbstractEventParser<EVENT> extends AbstractCanalLifeCycle implements
    CanalEventParser<EVENT> {
  ...
  protected Thread.UncaughtExceptionHandler handler = (t, e) -> logger.error(
      ...
      public void run() {
        ...
        while (running) {
          try {
            ...
            logger.warn("---> find start position successfully, {}",
                startPosition.toString() + " cost : "
                    + (end - start)
            ...
            if (!running) {
              if (!(e instanceof java.nio.channels.ClosedByInterruptException
                ...
                throw new CanalParseException(
                    String.format("dump address %s has an error, retrying. ",
                        runningInfo.getAddress().toString()), e);
              }
            } else {
              logger.error(String.format("dump address %s has an error, retrying. caused by ",
  ...
  protected boolean consumeTheEventAndProfilingIfNecessary(List<CanalEntry.Entry> entrys)
    ...
    boolean result = eventSink.sink(entrys, (runningInfo == null) ? null : runningInfo.getAddress(),
        destination);
  ...
  protected void processSinkError(Throwable e, LogPosition lastPosition, String startBinlogFile,
      Long startPosition) {
  ...
  private Map<String, List<String>> parseFieldFilterMap(String config) {
    ...
    if (StringUtils.isNotBlank(config)) {
      for (String filter : config.split(",")) {
        ...
        map.put(filterConfig[0].trim().toUpperCase(),
            Arrays.asList(filterConfig[1].trim().toUpperCase().split("/")));
      }
    }
