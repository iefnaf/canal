...
public class HttpHelper {
    ...
    try (CloseableHttpResponse response = httpclient.execute(httpGet, context)) {
      ...
      if (statusCode == HttpStatus.SC_OK) {
        ...
        throw new RuntimeException(
            "requestGet remote error, url=" + uri.toString() + ", code=" + statusCode
                + ", error msg=" + errorMsg);
      }
    } finally {
  ...
  private static String getIgnoreCerf(String url, CookieStore cookieStore,
  ...
  private static String postIgnoreCerf(String url, CookieStore cookieStore,
      Map<String, String> params, int timeout) {
    long start = System.currentTimeMillis();
    HttpClientBuilder builder = HttpClientBuilder.create();
    builder.setMaxConnPerRoute(50);
  ...
  public static String post(String url, CookieStore cookieStore, Map<String, String> params,
      int timeout) {
    url = url.trim();
    // 支持采用https协议，忽略证书
    if (url.startsWith("https")) {
    ...
    try {
      ...
      if (statusCode == HttpStatus.SC_OK) {
        ...
        String curlRequest = getCurlRequest(url, cookieStore, params, cost);
        throw new RuntimeException(
            "requestPost remote error, request : " + curlRequest + ", statusCode="
  ...
  public static void printCurlRequest(String url, CookieStore cookieStore,
      Map<String, String> params, long cost) {
    logger.warn(getCurlRequest(url, cookieStore, params, cost));
  }

  private static String getCurlRequest(String url, CookieStore cookieStore,
      Map<String, String> params, long cost) {
    if (params == null) {
      return "curl '" + url + "'\ncost : " + cost;
    } else {
