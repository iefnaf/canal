...
public class LogEventConvert extends AbstractCanalLifeCycle implements BinlogParser<LogEvent> {
  ...
  private Entry parseQueryEvent(QueryLogEvent event, boolean isSeek) {
    ...
    if (StringUtils.startsWithIgnoreCase(queryString, XA_START)) {
      ...
      boolean isDml = (type == EventType.INSERT || type == EventType.UPDATE
  ...
  private boolean processFilter(String queryString, DdlResult result) {
    ...
    if (result.getType() == EventType.ALTER || result.getType() == EventType.ERASE
      ...
      if (!filterQueryDdl && (StringUtils.isEmpty(tableName)
          || (result.getType() == EventType.RENAME && StringUtils.isEmpty(
        ...
        if (nameBlackFilter != null && nameBlackFilter.filter(name)) {
          if (result.getType() == EventType.RENAME) {
            ...
            if (nameBlackFilter != null
                && nameBlackFilter.filter(
                result.getOriSchemaName() + "." + result.getOriTableName())) {
              return true;
            }
          } else {
            // 其他情况返回null
  ...
  public TableMeta parseRowsEventForTableMeta(RowsLogEvent event) {
    ...
    boolean isRDSHeartBeat =
        tableMetaCache.isOnRDS() && isRDSHeartBeat(table.getDbName(), table.getTableName());
    ...
    if (isRDSHeartBeat) {
      ...
      tableMeta = new TableMeta(table.getDbName(), table.getTableName(),
          Arrays.asList(idMeta, typeMeta));
    } else if (isHeartBeat) {
  ...
  public Entry parseRowsEvent(RowsLogEvent event, TableMeta tableMeta) {
    ...
    try {
      ...
      if (tableError) {
        ...
        logger.warn("table parser error : {}storeValue: {}", entry.toString(),
            rowChange.toString());
        return null;
      } else {
  ...
  private EntryPosition createPosition(LogHeader logHeader) {
    return new EntryPosition(logHeader.getLogFileName(),
        logHeader.getLogPos() - logHeader.getEventLen(), // startPos
        logHeader.getWhen() * 1000L,
        logHeader.getServerId()); // 记录到秒
  }

  private boolean parseOneRow(RowData.Builder rowDataBuilder, RowsLogEvent event,
      RowsLogBuffer buffer, BitSet cols,
    ...
    if (tableMeta != null && columnInfo.length > tableMeta.getFields().size()) {
      ...
      if (!existRDSNoPrimaryKey) {
        ...
        tableMeta = getTableMeta(event.getTable().getDbName(), event.getTable().getTableName(),
            false, position);// 强制重新获取一次
        if (tableMeta == null) {
          tableError = true;
          if (!filterTableError) {
      ...
      if (buffer.isNull()) {
        ...
        switch (javaType) {
          ...
          case Types.BIGINT:
            ...
            boolean isUnsigned = (fieldMeta != null ? fieldMeta.isUnsigned()
                : (existOptionalMetaData ? info.unsigned : false));
              ...
              switch (buffer.getLength()) {
                ...
                case 8: /* MYSQL_TYPE_LONGLONG */
                  columnBuilder.setValue(
                      BIGINT_MAX_VALUE.add(BigInteger.valueOf(number.longValue()))
                          .toString());
  ...
  private Header createHeader(LogHeader logHeader, String schemaName, String tableName,
      EventType eventType) {
    ...
    for (Column column : bfColumns) {
      if (column.getIndex() == index) {// 比较before / after的column index
        if (column.getIsNull() && newValue == null) {
          // 如果全是null
          return false;
        } else if (newValue != null && (!column.getIsNull() && column.getValue()
            .equals(newValue))) {
  ...
  private TableMeta getTableMeta(String dbName, String tbName, boolean useCache,
    ...
    try {
      ...
      if (filterTableError) {
        if (StringUtils.contains(message, "errorNumber=1146") && StringUtils.contains(message,
            "doesn't exist")) {
          return null;
   ...
   * 字段过滤判断
   */
  private boolean needField(List<String> fieldList, List<String> blackFieldList,
      String columnName) {
