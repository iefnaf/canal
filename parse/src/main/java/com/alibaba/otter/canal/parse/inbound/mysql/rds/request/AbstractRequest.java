...
public abstract class AbstractRequest<T> {
  ...
  private String concatQueryString(Map<String, String> parameters)
  ...
  private final HttpResponse executeHttpRequest(HttpGet getMethod, String host) throws Exception {
    ...
    if (statusCode != HttpResponseStatus.OK.code()
        && statusCode != HttpResponseStatus.PARTIAL_CONTENT.code()) {
      String result = EntityUtils.toString(response.getEntity());
      throw new RuntimeException(
          "return error !" + response.getStatusLine().getReasonPhrase() + ", " + result);
    }
    return response;
  }
