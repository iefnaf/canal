...
public abstract class AbstractMQProducer implements CanalMQProducer {
  ...
  private void loadCanalMqProperties(Properties properties) {
    String flatMessage = PropertiesUtils.getProperty(properties,
        CanalConstants.CANAL_MQ_FLAT_MESSAGE);
    if (!StringUtils.isEmpty(flatMessage)) {
      mqProperties.setFlatMessage(Boolean.parseBoolean(flatMessage));
    }

    String databaseHash = PropertiesUtils.getProperty(properties,
        CanalConstants.CANAL_MQ_DATABASE_HASH);
    if (!StringUtils.isEmpty(databaseHash)) {
      mqProperties.setDatabaseHash(Boolean.parseBoolean(databaseHash));
    }
    String filterTranEntry = PropertiesUtils.getProperty(properties,
        CanalConstants.CANAL_FILTER_TRANSACTION_ENTRY);
    if (!StringUtils.isEmpty(filterTranEntry)) {
      mqProperties.setFilterTransactionEntry(Boolean.parseBoolean(filterTranEntry));
    }
    String parallelBuildThreadSize = PropertiesUtils.getProperty(properties,
        CanalConstants.CANAL_MQ_BUILD_THREAD_SIZE);
    ...
    String parallelSendThreadSize = PropertiesUtils.getProperty(properties,
        CanalConstants.CANAL_MQ_SEND_THREAD_SIZE);
    ...
    String batchSize = PropertiesUtils.getProperty(properties,
        CanalConstants.CANAL_MQ_CANAL_BATCH_SIZE);
    if (!StringUtils.isEmpty(batchSize)) {
      mqProperties.setBatchSize(Integer.parseInt(batchSize));
    }
    String timeOut = PropertiesUtils.getProperty(properties,
        CanalConstants.CANAL_MQ_CANAL_GET_TIMEOUT);
    if (!StringUtils.isEmpty(timeOut)) {
      mqProperties.setFetchTimeout(Integer.parseInt(timeOut));
    }
    String accessChannel = PropertiesUtils.getProperty(properties,
        CanalConstants.CANAL_MQ_ACCESS_CHANNEL);
    if (!StringUtils.isEmpty(accessChannel)) {
      mqProperties.setAccessChannel(accessChannel);
    }
    String aliyunAccessKey = PropertiesUtils.getProperty(properties,
        CanalConstants.CANAL_ALIYUN_ACCESS_KEY);
    if (!StringUtils.isEmpty(aliyunAccessKey)) {
      mqProperties.setAliyunAccessKey(aliyunAccessKey);
    }
    String aliyunSecretKey = PropertiesUtils.getProperty(properties,
        CanalConstants.CANAL_ALIYUN_SECRET_KEY);
