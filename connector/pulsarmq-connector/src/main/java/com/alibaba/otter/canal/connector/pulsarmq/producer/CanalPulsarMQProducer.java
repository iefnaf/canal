...
public class CanalPulsarMQProducer extends AbstractMQProducer implements CanalMQProducer {
    ...
    if (StringUtils.isNotEmpty(pulsarMQProducerConfig.getAdminServerUrl())) {
      try {
        pulsarAdmin = PulsarAdmin.builder()
            .serviceHttpUrl(pulsarMQProducerConfig.getAdminServerUrl()).build();
      } catch (PulsarClientException e) {
        throw new RuntimeException(e);
      }
    }
  ...
  private void loadPulsarMQProperties(Properties properties) {
    ...
    String serverUrl = PropertiesUtils.getProperty(properties,
        PulsarMQConstants.PULSARMQ_SERVER_URL);
    if (!StringUtils.isEmpty(serverUrl)) {
      tmpProperties.setServerUrl(serverUrl);
    }

    String roleToken = PropertiesUtils.getProperty(properties,
        PulsarMQConstants.PULSARMQ_ROLE_TOKEN);
    ...
    String adminServerUrl = PropertiesUtils.getProperty(properties,
        PulsarMQConstants.PULSARMQ_ADMIN_SERVER_URL);
    ...
    if (!mqProperties.isFlatMessage()) {
      ...
      if (destination.getPartitionHash() != null && !destination.getPartitionHash().isEmpty()) {
        for (MQMessageUtils.EntryRowData r : datas) {
          ...
          // 串行分区
          com.alibaba.otter.canal.protocol.Message[] messages = MQMessageUtils.messagePartition(
              datas,
              message.getId(),
              partitionNum,
   ...
   */
  private void sendMessage(String topic, int partitionNum,
      com.alibaba.otter.canal.protocol.Message msg) {
    Producer<byte[]> producer = getProducer(topic);
    byte[] msgBytes = CanalMessageSerializerUtil.serializer(msg,
    ...
    public int choosePartition(Message<?> msg, TopicMetadata metadata) {
      ...
      if (!StringUtils.isEmpty(partitionStr)) {
        try {
          ...
          logger
              .warn("Parse msg {} property failed for value: {}", MSG_PROPERTY_PARTITION_NAME,
                  partitionStr);
        }
      }
      // topic创建时设置的分区数
