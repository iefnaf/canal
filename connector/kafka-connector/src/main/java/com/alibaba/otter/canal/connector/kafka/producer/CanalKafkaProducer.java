...
public class CanalKafkaProducer extends AbstractMQProducer implements CanalMQProducer {
  ...
  private void loadKafkaProperties(Properties properties) {
    ...
    doMoreCompatibleConvert("canal.mq.kafka.kerberos.krb5.file", "kafka.kerberos.krb5.file",
        properties);
    doMoreCompatibleConvert("canal.mq.kafka.kerberos.jaas.file", "kafka.kerberos.jaas.file",
        properties);
    ...
    String kerberosEnabled = PropertiesUtils.getProperty(properties,
        KafkaConstants.CANAL_MQ_KAFKA_KERBEROS_ENABLE);
    if (!StringUtils.isEmpty(kerberosEnabled)) {
      kafkaProducerConfig.setKerberosEnabled(Boolean.parseBoolean(kerberosEnabled));
    }
    String krb5File = PropertiesUtils.getProperty(properties,
        KafkaConstants.CANAL_MQ_KAFKA_KERBEROS_KRB5_FILE);
    if (!StringUtils.isEmpty(krb5File)) {
      kafkaProducerConfig.setKrb5File(krb5File);
    }
    String jaasFile = PropertiesUtils.getProperty(properties,
        KafkaConstants.CANAL_MQ_KAFKA_KERBEROS_JAAS_FILE);
  ...
  private List<Future> send(MQDestination mqDestination, String topicName, Message message,
      boolean flat) {
    ...
    Integer partitionNum = MQMessageUtils.parseDynamicTopicPartition(topicName,
        mqDestination.getDynamicTopicPartitionNum());
    ...
    if (!flat) {
      ...
      for (FlatMessage flatMessage : flatMessages) {
        if (mqDestination.getPartitionHash() != null && !mqDestination.getPartitionHash()
            .isEmpty()) {
