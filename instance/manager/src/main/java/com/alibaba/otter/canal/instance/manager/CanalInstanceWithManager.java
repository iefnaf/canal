...
public class CanalInstanceWithManager extends AbstractCanalInstance {
  ...
  public CanalInstanceWithManager(Canal canal, String filter) {
    ...
    logger.info("init CanalInstance for {}-{} with parameters:{}", canalId, destination,
        parameters);
    // 初始化报警机制
    initAlarmHandler();
    // 初始化metaManager
  ...
  protected void initAlarmHandler() {
    ...
    if (alarmHandlerClass == null || alarmHandlerPluginDir == null) {
      ...
      try {
        ...
        Class<CanalAlarmHandler> _alarmClass = (Class<CanalAlarmHandler>) currentClassLoader.loadClass(
            alarmHandlerClass);
    ...
    logger.info("init alarmHandler end! \n\t load CanalAlarmHandler:{} ",
        alarmHandler.getClass().getName());
  }
  ...
  protected void initMetaManager() {
    ...
    logger.info("init metaManager end! \n\t load CanalMetaManager:{} ",
        metaManager.getClass().getName());
  }
  ...
  protected void initEventParser() {
        ...
        for (List<DataSourcing> groupDbAddress : groupDbAddresses) {
          if (lastType != null && !lastType.equals(groupDbAddress.get(i).getType())) {
            throw new CanalException(
                String.format("master/slave Sourcing type is unmatch. %s vs %s",
                    lastType,
    ...
    logger.info("init eventParser end! \n\t load CanalEventParser:{}",
        eventParser.getClass().getName());
  }

  private CanalEventParser doInitEventParser(SourcingType type,
      List<InetSocketAddress> dbAddresses) {
    ...
    if (type.isMysql()) {
      ...
      // 网络相关参数
      mysqlEventParser.setDefaultConnectionTimeoutInSeconds(
      ...
      if (!CollectionUtils.isEmpty(parameters.getPositions())) {
        EntryPosition masterPosition = JsonUtils.unmarshalFromString(
        ...
        if (parameters.getPositions().size() > 1) {
          EntryPosition standbyPosition = JsonUtils.unmarshalFromString(
      ...
      localBinlogEventParser.setDetectingIntervalInSeconds(
          parameters.getDetectingIntervalInSeconds());
  ...
  protected CanalHAController initHaController() {
    ...
    if (haMode.isHeartBeat()) {
      ...
      ((HeartBeatHAController) haController).setDetectingRetryTimes(
    ...
    logger.info("init haController end! \n\t load CanalHAController:{}",
        haController.getClass().getName());
