...
public class JsonDiffConversion {
  ...
  public static StringBuilder print_json_diff(LogBuffer buffer, long len, String columnName,
      int columnIndex,
      String charsetName) {
    ...
    while (buffer.hasRemaining()) {
      ...
      if (!buffer.hasRemaining() || operation_names.get(diff_i + 1) != operation_names.get(
          diff_i)) {
        builder.append(")");
      }
