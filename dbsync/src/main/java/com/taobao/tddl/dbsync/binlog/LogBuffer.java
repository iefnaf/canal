...
public class LogBuffer {
  ...
  public final int getUint24(final int pos) {
    ...
    byte[] buf = buffer;
    return (0xff & buf[position]) | ((0xff & buf[position + 1]) << 8) | ((0xff & buf[position + 2])
        << 16);
  }
  ...
  public final int getUint24() {
    ...
    byte[] buf = buffer;
    return (0xff & buf[position++]) | ((0xff & buf[position++]) << 8) | ((0xff & buf[position++])
        << 16);
  }
  ...
  public final int getBeUint24(final int pos) {
    ...
    byte[] buf = buffer;
    return (0xff & buf[position + 2]) | ((0xff & buf[position + 1]) << 8) | ((0xff & buf[position])
        << 16);
  }
  ...
  public final int getBeUint24() {
    ...
    byte[] buf = buffer;
    return ((0xff & buf[position++]) << 16) | ((0xff & buf[position++]) << 8) | (0xff
        & buf[position++]);
  }
  ...
  public final int getBeInt32() {
    ...
    byte[] buf = buffer;
    return ((buf[position++]) << 24) | ((0xff & buf[position++]) << 16) | ((0xff & buf[position++])
        << 8)
        | (0xff & buf[position++]);
  }
  ...
  public static final int powers10[] = {1, 10, 100, 1000, 10000, 100000, 1000000, 10000000,
  ...
  public final BigDecimal getDecimal(final int pos, final int precision, final int scale) {
    ...
    final int binSize =
        intg0 * SIZE_OF_INT32 + dig2bytes[intg0x] + frac0 * SIZE_OF_INT32 + dig2bytes[frac0x];
   ...
   */
  private final BigDecimal getDecimal0(final int begin, final int intg, final int frac,
      final int intg0,
    ...
    if (intg0x != 0) {
      ...
      if (x < 0 || x >= powers10[intg0x + 1]) {
        throw new IllegalArgumentException(
            "bad format, x exceed: " + x + ", " + powers10[intg0x + 1]);
      }
      if (x != 0 /* !digit || x != 0 */) {
